...
from app import make_move, get_winner_loser
...
class MakeMoveTestCase(unittest.TestCase):
    ...
    def test_make_move_success(self):
        for test in self.success_test_params:
            ...
            actual = make_move(test[KEY_INPUT_SQUARE], test[KEY_INPUT_TURNX],
                               test[KEY_INPUT_BOARD])

            self.assertEqual(len(expected), len(actual))
            self.assertEqual(expected[test[KEY_INPUT_SQUARE] - 9],
                             actual[test[KEY_INPUT_SQUARE] - 9])
            self.assertEqual(expected[test[KEY_INPUT_SQUARE] - 1],
                             actual[test[KEY_INPUT_SQUARE] - 1])
            self.assertEqual(expected[test[KEY_INPUT_SQUARE] - 2],
                             actual[test[KEY_INPUT_SQUARE] - 2])
...
class GetWinnerLoserTestCase(unittest.TestCase):
    ...
    def test_make_move_success(self):
        for test in self.success_test_params:
            expected = test[KEY_EXPECTED]
            actual = get_winner_loser(test[KEY_INPUT_WINNER],
                                      test[KEY_INPUT_USERS])
